...
public final class Source {
    ...
    public static class UndoableTodoList {
        ...
        public static final String[] GENERATED = {
                ...
                "import info.izumin.android.droidux.processor.fixture.TodoList;",
                "",
                "public final class DroiduxUndoableTodoListStore extends Store<TodoList> {",
                "    private final UndoableTodoListReducer undoableTodoListReducer;",
                "    private final History<TodoList> history;",
                "",
                "    protected DroiduxTodoListStore(Builder builder) {",
                "        super(builder);",
                "        this.undoableTodoListReducer = builder.undoableTodoListReducer;",
                "        this.history = new History<>(builder.todoList);",
                "        setState(history.getPresent());",
                "    }",
                "",
                "    @Override",
                "    protected void dispatchToReducer(Action action) {",
                ...
                "        TodoList result = null;",
                "        if (actionClass.isAssignableFrom(AddTodoItemAction.class)) {",
                "            result = undoableTodoListReducer.onAddItem(getState(), (AddTodoItemAction) action);",
                "            history.insert(result);",
                "        }",
                "        if (actionClass.isAssignableFrom(CompleteTodoItemAction.class)) {",
                "            result = undoableTodoListReducer.onCompleteItem(getState(), (CompleteTodoItemAction) action);",
                "            history.insert(result);",
                "        }",
                "        if (actionClass.isAssignableFrom(HistoryAction.class)) {",
                "            HistoryAction historyAction = (HistoryAction) action;",
                "            if (historyAction.isAssignableTo(this)) {",
                "                result = historyAction.handle(history);",
                "            }",
                "        }",
                "        if (result != null) {",
                "            setState(result);",
